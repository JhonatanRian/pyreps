...
def test_xlsx_options_validation_gaps() -> None:
    ...
    with pytest.raises(
        ReportError, match=re.escape("metadata['xlsx'] must be a mapping")
    ):
        XlsxRenderOptions.from_metadata({"xlsx": "invalid"})

    # metadata['xlsx']['columns'] not a mapping
    with pytest.raises(
        ReportError, match=re.escape("metadata['xlsx']['columns'] must be a mapping")
    ):
        XlsxRenderOptions.from_metadata({"xlsx": {"columns": []}})

    # column label not a string
    with pytest.raises(
        ReportError, match=re.escape("metadata['xlsx']['columns'] keys must be strings")
    ):
    ...
    with pytest.raises(
        ReportError,
        match=re.escape("metadata['xlsx']['columns']['A'] must be a mapping"),
    ):
...
def test_sheet_name_validation_gaps() -> None:
    # Empty sheet name
    with pytest.raises(
        ReportError, match=re.escape("sheet_name'] must be a non-empty string")
    ):
        XlsxRenderOptions(sheet_name="  ")

    # Too long
    with pytest.raises(
        ReportError, match=re.escape("exceeds Excel limit of 31 characters")
    ):
    ...
    for char in "\\/*?[]:":
        with pytest.raises(
            ReportError, match=re.escape("contains invalid Excel characters")
        ):
            XlsxRenderOptions(sheet_name=f"Sheet{char}")

    # Apostrophe
    with pytest.raises(
        ReportError, match=re.escape("cannot start or end with apostrophe")
    ):
...
def test_xlsx_column_options_validation_gaps() -> None:
    ...
    with pytest.raises(
        ReportError, match=re.escape("min_width cannot be greater than max_width")
    ):
